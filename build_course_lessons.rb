...
POOL = lessons.flat_map do |lesson|
  lesson["vocabulary"].map { |word| word.slice("zh", "en", "ru", "pos", "zhuyin").merge("lesson" => lesson["number"]) }
end.freeze

REPLIES = lessons.flat_map do |lesson|
...
STREET = lessons.flat_map { |lesson| Array(lesson["usage"]).map { |entry| entry.merge("lesson" => lesson["number"]) } }.freeze

def gloss_of(word) = {"en" => word["en"], "ru" => word["ru"]}

def near(rng, candidates, correct, distinct)
  candidates.reject { |candidate| distinct.call(candidate, correct) }.map { |candidate|
    score = 0
  ...
  }.sort_by { |score, candidate| [-score, candidate["zh"].to_s] }.map(&:last)
end
...
def build_tasks(lesson, analyzer, rng, want_street: true)
  ...
  2.times do
    word = fresh.call
    tasks << choice(rng, "meaning", word, pick(rng, POOL, word, SAME_GLOSS), ->(other) { gloss_of(other) }, "zh" => word["zh"])
  end

  word = fresh.call
  tasks << choice(rng, "word", word, pick(rng, POOL, word, SAME_WORD), ->(other) { other["zh"] }, "gloss" => gloss_of(word))
  ...
  if readable.any?
    ...
    used << word["zh"]
    tasks << choice(rng, "reading", word, pick(rng, POOL, word, SAME_WORD), ->(other) { other["zh"] }, "zh" => word["zhuyin"])
  end
  ...
  lines.each do |line|
    ...
    hit = vocab.find do |entry|
      entry["zh"].length >= 2 && line["zh"].scan(entry["zh"]).size == 1 && cloze.none? { |_, taken| taken["zh"] == entry["zh"] }
    end
    cloze << [line, hit] if hit
  end
  cloze.each do |line, word|
    tasks << choice(
      rng, "cloze", word, pick(rng, POOL, word, SAME_WORD), ->(other) { other["zh"] },
      "zh" => line["zh"].sub(word["zh"], "＿＿"), "gloss" => {"en" => line["en"], "ru" => line["ru"]}
    )
  end
  ...
  if turns.any?
    ...
    tasks << choice(
      rng, "reply", {"zh" => answer["zh"]},
      pick(rng, others, {"zh" => answer["zh"], "lesson" => lesson["number"]}, SAME_LINE),
      ->(reply) { reply["zh"] }, "zh" => ask["zh"], "gloss" => {"en" => ask["en"], "ru" => ask["ru"]}
    )
  end
  ...
  if short.any?
    ...
    if chunks.size.between?(3, 8)
      tasks << {
        "kind" => "order", "gloss" => {"en" => source["en"], "ru" => source["ru"]},
        "chunks" => chunks, "order" => shuffled(rng, chunks.size), "answer" => 0
      }
    end
  end
  ...
  if want_street && own.any?
    ...
    distractors = pick(rng, others, entry, ->(a, b) { a["street"] == b["street"] }, 3).map { |row| {"zh" => row["street"]} }
    if distractors.size == 3
      tasks << choice(rng, "street", {"zh" => entry["street"]}, distractors, ->(row) { row["zh"] }, "zh" => entry["standard"])
    end
  end
...
stages.each do |stage|
  ...
  drawn = own.flat_map { |lesson| build_tasks(lesson, analyzer, Random.new(lesson["number"] * 31 + stage["order"].to_i), want_street: false) }
...
puts "lessons #{lessons.size}, tasks #{lessons.sum { |lesson| lesson["exercises"].size }}, exams #{stages.sum { |stage| stage["exam"].size }}"
